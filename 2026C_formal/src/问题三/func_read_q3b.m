function [price_v, load_m, pv_m, day_list, fc3] = func_read_q3b(PROJ_ROOT)
%FUNC_READ_Q3B  读取问题三第二版所需的输入数据（附件1 / 附件2 / 附件3）
%
%   附件1：电价与典型日曲线（每天相同，144 槽）——读法与问题二完全一致
%   附件2：小区负载与光伏发电实际功率（365 天 × 144 槽）——读法与问题二完全一致
%   附件3：每天 0:00 / 6:00 / 12:00 / 18:00 发布的未来 24 小时整点光伏预报
%
%   附件3 索引口径（裁决 A5，已由三组判据独立复核，见 outputs/preprocess_log_q3b.txt）：
%       发布时刻 τ 的"预报第 k 小时" ↔ 区间 [τ+(k−1)h, τ+kh)
%   日期只写在每日首行，故按行号整除 4 定位，不按日期字符串匹配。
%
%   输入  PROJ_ROOT  题目根目录
%   输出  price_v  144×1      电价 元/kWh（已按数据索引归位）
...
fc3 = zeros(D, 4, 24);
for j = 1:4
    blk = raw(2 + (0:D-1)*4 + (j-1), 3:26);      % 第 j 个发布时刻的整块
