...
ePV3 = nan(D, T, 4);
% ★ 必须按行主序展平：pv_m(:) 是列主序（先跑完 365 天再换下一槽），
%   配合下面的 (r−1)*T+t 索引会取到别的日子——长度对得上、均值也接近，
%   只有分布被整体错配，表现为"情景光度虚高"这类看起来只是数值偏大的错。
...
% 但白天时段会出现明显偏正的均值——这条断言专门拦这一类静默错配。
for j = 1:4
