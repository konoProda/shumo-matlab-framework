...
> 依据：《问题二_7日滚动SAA两阶段MILP_给编程手.md》与 `outputs/裁决与映射/decisions_q2c.md`
...
| 裁决 | 落地文件 | 行号 |
|---|---|---|
| C6 第一阶段非预见性 | `src/问题二/func_build_q2c.m` | 第 44 行（`aux.iGP`）、第 152 行（当天块引用同一列） |
| C7 第二阶段含 W | `src/问题二/func_build_q2c.m` | 第 33~35、147 行 |
| C8 未来日临时计划 | `src/问题二/func_build_q2c.m` | 第 149~153 行 |
| C9 连接约束 | `src/问题二/func_build_q2c.m` | 第 145~154 行 |
| C11 目标权重 | `src/问题二/func_build_q2c.m` | 第 70~78 行 |
| C12 只执行当天 | `src/问题二/func_roll_q2c.m` | 第 142~144 行 |
| E1 计划不可改 | `src/问题二/func_exec_q2c.m` | 第 70 行（按完整计划量计费） |
| 断点续跑 | `src/问题二/func_roll_q2c.m` | 第 40~67、157~160 行（行号已于 /report 阶段按当前源码复核） |
