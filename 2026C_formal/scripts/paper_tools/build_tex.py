...
def esc_text(s):
    ...
    只转义**尚未转义**的特殊字符。早先写成 s.replace('%', r'\\%')，
    对初稿里本来就是 `\\%` 的地方又转一次，变成 `\\\\%`——在行内公式
    `\\(12.5\\\\%\\)` 里 `\\\\` 是换行命令，直接编译报错 "Missing $ inserted"。
    """
    # 只处理 %：它不转义会静默吞行，是唯一的真雷。
    # & _ # 不能盲转——文本与表格单元里含 `\\(x_t\\)` 这类行内公式，
    # 把下划线转成 \\_ 会直接破坏数学，属于"修一个坏一个"。
    return re.sub(r'(?<!\\)%', r'\\%', s)
