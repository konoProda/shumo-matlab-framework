...
价格情景为 \(\pi^{(\omega)}
=
\hat\pi+e^\pi\)。 历史残差叠加后可能出现负价格，因此模型保留负价格情景，不将其人工截断为 0。
...
0:00 时负荷采用问题二自建预测，光伏采用附件 3 的 0:00 最新预报，电价采用 0:00 价格预测。

6:00、12:00、18:00：

三个调整时点均保持负荷中心预测为当天 0:00 版本，把光伏中心预测切换为当前时刻附件 3 的最新预报，允许电价预测利用已发生的真实价格作因果更新；同时锁定已执行时段，并以当前真实储电量作为新阶段初值。
...
C_d^{em,act}=\sum_t 5\pi^{act}_{d,t}H^{act}_{d,t}\Delta t.
\end{gathered}
\end{equation}
...
结果分析围绕六个问题展开：波动价格相对于固定价格如何改变总成本；储能是否表现出更明显的低价充电、高价放电；电价预测误差造成多少额外成本；Q4-3 的日内光伏与价格更新是否进一步降低紧急购电或调整成本；负价格情景下 H-1/H-2 是否被实际激活；以及价格峰谷预测偏差如何影响储能调度时机。
...
H-2 启用后，应有 \(\#\{t:\Delta_t^+>0\text{ 且 }\Delta_t^->0\}=0\)。 最大等式残差：\(6.96\times10^{-10}.\)最大整数间隙：
