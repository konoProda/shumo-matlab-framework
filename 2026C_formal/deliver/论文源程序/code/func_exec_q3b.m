...
%
%   与问题二 func_exec_q2c 的规则逐条相同，只是允许一次执行一段（段长 = numel(Gplan)），
%   以便按"求解—执行—更新—再求解"的顺序推进并取得各阶段边界处的真实储电量。
%   不修改 func_exec_q2c 本身（问题二产物保持有效）。
%
%   逐槽优先级：
%     ① 光伏优先供负荷；② 当前生效正常购电补负荷；③ 仍不足先放电、再紧急购电；
%     ④ 负荷满足后光伏余电优先充储能；⑤ 其次用剩余正常购电充储能；
%     ⑥ 剩余光伏记 V、剩余已购电记 W（分开）；⑦ 同槽不得同时充放；⑧ 紧急购电不给储能充电。
%
%   输入  Gplan    n×1 该段的最终生效正常购电功率 kW（本段内已锁定）
