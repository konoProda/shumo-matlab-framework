%% 四个指定日期的计划与紧急购电
% 输入 data.csv / 输出 问题三 指定日期四阶段轨迹.png、问题三 指定日期四阶段轨迹.pdf
...
POS   = [0.085 0.560 0.360 0.290; 0.555 0.560 0.360 0.290; ...
         0.085 0.140 0.360 0.290; 0.555 0.140 0.360 0.290];
LGPOS = [0.250 0.930 0.500 0.040];            % 图例（figure 归一化坐标，置于图幅顶部）
TITLEY = 0.008;                                % 图名 y（底部）
HW    = 1/6;                                   % 每槽小时数
...
for k = 1:numel(DATES)
    ...
    % 紧急购电用连线而非柱：该量平时为 0、只在个别时段跳起，柱形在此量级下看不出来
    plot(ax, t, M.em_kwh/HW, '-', 'LineWidth', 1.8, 'Color', func_fig_pal(4));
