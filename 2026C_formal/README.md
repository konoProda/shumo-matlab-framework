...
├── outputs/            结果、日志、裁决文档、备份（组内）
├── data/               原始附件与预处理数据
├── tests/              测试脚本（组内）
├── dev_tools/          Python 辅助脚本（组内）
├── 建模手交接/          与建模手的往来文件（交付建模手 / 接收自建模手）
├── paper/              论文写作资源
│
...
| 目录 | 要点 | 交付？ |
|---|---|---|
| `deliver/` | 每问一个「问题X对应的代码与说明」文件夹（入口 `.m` + 子函数副本 + `问题X_代码说明.docx` + 题目要求的结果表 `result*.xlsx`）；顶层每问一张关键结果图 `.jpg` | ✅ 唯一对外 |
...
| `outputs/` | 顶层为**运行直接读写**的结果/断点/日志，其余按类分目录（`裁决与映射/`、`测试记录/`、`分析记录/`、`统计中间件/`、`历史日志/`、`_旧版_勿引用/`、`_备份/`）；详见 [`outputs/README.md`](outputs/README.md) | ❌ |
