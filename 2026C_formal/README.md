...
├── src/                求解代码，按问分目录（问题一/二/三/四/共用）
├── scripts/            后处理与打包脚本
├── figures/            图件，一图一自包含文件夹（脚本 + data.csv + PNG）
├── outputs/            结果、日志、过程文档（组内）
├── data/               原始附件与预处理数据
├── tests/              测试脚本（组内）
...
| `figures/` | 一图一文件夹；**plot 脚本只读 `data.csv`，不做计算**；`_未入选_勿引用/` 为未入选的 4 张（结论仍引用，图不配） | ✅ 经筛选 |
| `outputs/` | 顶层为**运行直接读写**的结果/断点/日志，其余按类分目录（`裁决与映射/`、`测试记录/`、`分析记录/`、`统计中间件/`、`历史日志/`）；详见 [`outputs/README.md`](outputs/README.md) | ❌ |
| `scripts/` | 后处理流水线与打包；详见 [`scripts/README.md`](scripts/README.md) | ❌ |
| `建模手交接/` | `交付建模手/`（确认单、回执）、`接收自建模手/`（来文，一律 `问题X_` 前缀） | ❌ |

**不看的东西**：路径中含 `_勿引用` 的目录或文件均为历史留档，**论文与交付一律不引用**。
...
**尚缺（编程手不生成，收尾时请全队补入 `deliver/`）**：`论文.pdf`、`AI工具使用说明.pdf`。

2026-09-14 交付完成，过程备份与旧版本归档已清除（107M / 14 项）。
删除清单、保留项与范围见 [`outputs/历史日志/清理记录_20260914.txt`](outputs/历史日志/清理记录_20260914.txt)；
清除前的完整工作区保存在提交 `fbe8845`，任何已删文件均可用 `git show fbe8845:<路径>` 取回。
