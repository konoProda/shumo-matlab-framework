...
每天仍划分为 \(T=144\)、每段 \(\Delta t=1/6\) h。 2025 年 1 月作为已知数据预热期，直接使用真实负荷和真实光伏建立预测历史；正式滚动规划从 2025 年 2 月 1 日开始。
...
每天 \(d\) 只使用 \(d\) 日以前已经发生的历史真实数据建立负荷和光伏中心预测，记为 \(\hat L_{\tau,t}^{(d)},, \widehat{PV}_{\tau,t}^{(d)}\)。 在基础点预测上保留偏差校正，以降低系统性供电不足风险。
...
真实运行时按固定优先级依次执行：光伏优先满足负荷，不足部分由计划购电补充，再不足时储能放电，储能仍不足则紧急购电；负荷满足后，富余光伏优先充电，再利用剩余计划电给储能充电，剩余光伏记为 \(V\)、剩余已购正常电记为 \(W\)，且同一时段不得同时充放电。
...
当 \(K=4\) 时， \(Z=14\,806\,890.28\ \mathrm{元},, Q_{em}=269\,240.9\ \mathrm{kWh}\)。 当 \(K=8\) 时，
...
不带偏差校正时总费用为 14\,671\,204.28 元、紧急购电量为 274\,289.2 kWh；带偏差校正后总费用为 14\,806\,890.28 元、紧急购电量为 269\,240.9 kWh，即总费用增加约 0.92\%，紧急购电量下降约 1.8\%。因此偏差校正同时作用于经济性与供电可靠性，是两者之间的折中。