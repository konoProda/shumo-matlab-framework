...
正式情景数取 \(K=4\)， 稳定性分析取 \(K=8\)， 随机种子为 2026。
...
Q4-2 经联合情景展开后属于 7 日滚动两阶段 SAA-MILP，Q4-3 属于日内多阶段滚动 SAA-MILP。 两者的连续能量流变量与二元充放电状态、Q4-3 的调增调减互斥状态共同构成混合整数线性规划。
...
中心预测能够刻画日内峰谷形态，高低价时段基本对齐，该周预测对实测的 MAE 为 0.0371 元/kWh；但峰谷出现的具体时刻仍有偏差。
%FIG:END q4-week
...
H-2 启用后，应有 \(\#\{t:\Delta_t^+>0\text{ 且 }\Delta_t^->0\}=0\)。 最大等式残差：
